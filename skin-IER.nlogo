...
globals [
  ...
  S                              ; Number of agents in the avalanche
  H                              ; Total increment on expertise level
  rho                            ; mean rate of avalanche
  rho-average                    ; rho average: rho / ticks
...
firms-own [
  ...
  resistance                      ; the Firms resistance to adopt
...
to initialise-firm
  ...
  set abilities []
  set expertises []
  set resistance random-normal resistance-average resistance-standard-deviation
end
...
to go
  ...
  ask firms [ do-admin ]
  if ticks >= 100
  [set rho-average rho / ticks
;   ask firms [ ask partners [
;      show self
;      show myself] ]
  stop
  ]
...
to collaborate
  ...
  if not trading? [
    find-partners
    if any? partners  [ learn-from-partners-with-resistance ]
...
to find-partners
  ...
  ; now have a set of exactly max-partners candidates.  Select those that are acquaintance
  ; as actual partners
  set candidates candidates with [ acquaintance? myself ]
...
;firm procedure
to-report acquaintance? [ possible-partner ]
  ...
  ; social difference of two agents based on Boguna et. al. (2004) divided by scale
  let difference (length set-difference advert [advert] of possible-partner) / (length advert + length [advert] of possible-partner)
  let connection-probability 1 / (1 + difference ^ homophile)
  report random-float 0.5 < connection-probability
end
...
;firm procedure
to learn-from-partners-with-resistance
  ask partners [ merge-capabilities-with-resistance myself ]
  make-innovation-hypothesis
end

;firm procedure
to merge-capabilities-with-resistance [ other-firm ]
  add-capabilities-with-resistance other-firm
  ask other-firm [ add-capabilities-with-resistance myself ]
end
...
;firm procedure
to add-capabilities-with-resistance [ other-firm ]
  let my-position 0
  foreach [ih] of other-firm [ ?1 ->
    ...
    ifelse member? capability capabilities [
      ...
      if item ?1 [expertises] of other-firm - item my-position expertises >= resistance  [
        set S S + 1
        set H H + item ?1 [expertises] of other-firm - item my-position expertises
...
to create-network
  ...
  ask the-network [ add-members-with-resistance myself ]
...
; network procedure
to add-members-with-resistance [ the-firm ]
  ...
  ask the-firm [
    set net the-network
    merge-capabilities-with-resistance [ hq ] of the-network
  ]
...
"" ""
PENS
"default" 1.0 0 -12345184 true "" "plot rho"
